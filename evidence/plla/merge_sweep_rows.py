...
`P1.T23`, reused unchanged by `P1.T41`-`T43`.
...
Usage:
    python merge_sweep_rows.py <batch rows.jsonl> <runs.jsonl> [<attribution.json>]
"""
import json
import sys
...
def main(argv):
    ...
    attribution = json.loads(Path(argv[3]).read_text()) if len(argv) > 3 else None
    ...
    for line in batch_rows.read_text().splitlines():
        ...
        note = (f"P1.T23 batch A, point {point}, site {axis.site}: "
        ...
        if attr:
            note += (f" Moved {attr['moved_bits']} bits in tiles "
                     f"{attr['moved_tiles']} -> {attr['names']}; "
                     f"agrees with P1.T22 attrmap: {attr['verified_vs_t22']}.")
