...
**RETRACTED: "IOLOGIC configures on the A half only".** This row concluded that the B half of an IO tile has no IOLOGIC fuse table, from `ODDRC` on `IOB80[B]` decoding nothing: `db.shortval[247]['IOLOGICB']` holds 3 fuse coordinates against `IOLOGICA`'s 100, none set (the nine bits `ODDR` moves there are `pip`/routing, not IOLOGIC), and the GW5A-25A shows the same one-populated-one-stub shape per IO tile. **The observation is right and the reason published for it is wrong**, and it is the reason that constrained this phase's coverage to A-half balls (`AB16`, `AA15`) and was carried into `P3.T13` and `P3.T14` as an inherited limit.

`P3.T16a` measured the opposite four hours later. The 100-coordinate `IOLOGICB` table exists; it lives in the **adjacent aux cell**, and the vendor's own `OSER16` bitstream sets six bits in it -- row 108, col 51 ttyp 248 `IOLOGICB:100c set=0`, col 52 ttyp 247 `IOLOGICA:100c set=9`, col 53 ttyp 248 `IOLOGICB:100c set=6`. The stub this row read is the table that is *designed* to stay clear. `IOLOGICB` now carries `IOBB`'s `fuse_cell_offset` (`chipdb.gw5_displace_iologicb`), so a B-half gearbox is written where the vendor writes it instead of into the wrong tile.

**What that changes here, and what it does not.** The A-half rows are unaffected: the diff touches `IOLOGICB.fuse_cell_offset` alone, `IOLOGICA` is untouched, and every `ODDR`/`IDDR` point still decodes at `X79Y108/IOLOGICAO` and `X82Y108/IOLOGICAI` with `cells`/`attrs`/`conns` 0/0/0. What is **not** claimed is broad B-column coverage: the displacement is exercised against the vendor by exactly one design, the `OSER16` point, whose B half carries `OUTMODE=DDRENABLE`, `ISI` and `OCLKCE` and whose scoped fuses are bit-identical to the vendor's (`E1`, 0/0/0). A plain `ODDR` on a B-half ball is a named one-run follow-up, and until it is measured the displacement is claimed for `GW5AST-138C` alone -- `GW5A-25A` and `GW5AT-60B` keep the undisplaced offset and byte-identical chipdbs (`S3`).
