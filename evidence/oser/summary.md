...
Result: all six points stay `verdict: ok` with `cells`/`attrs`/`conns`
**0/0/0** and both decode checks `ok`, on the re-diff that also carries
`D108`'s IOB defaults. `ovideo-default` remains `E0` (no `CLS` placement to
pin, `EC9`); the other five are `E1`.
